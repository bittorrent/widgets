...
bt.Widget = (function() {
  ...
  Error = function(settings) {
    arguments.callee.check('Error', arguments, 'Os', 0);
    var e = (this instanceof Error) ? this : new Error(arguments);
    if ('object' === typeof settings && 'string' === typeof settings[0]) {
      e.message = settings[0];
    }
  ...
  return function(widgetName) {
    arguments.callee.check('Widget', arguments, 'S');
    ...
    this.addCallback = function(category, handler) {
      arguments.callee.check('addCallback', arguments, 'SF');
    ...
    this.dispatchCallbacks = function(category) {
      arguments.callee.check('dispatchCallbacks', arguments, 'S');
      var name = widgetName + ':' + category;
      if (!(name in eventHandlers)) return
    ...
    this.removeCallbacks = function(category) {
      arguments.callee.check('removeCallbacks', arguments, 'S');
