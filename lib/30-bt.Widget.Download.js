bt.Widget.Download = (function() {
      ...
      callbacks : {
        ...
        progressBar : (function() {
          var playClick = function() {
            _.sortBy(
              _.values(bt.torrent.get(this.url).file.all()),
              function(file){return 1-file.properties.get("size")}
            )[0].open();
          ...
          return function() {
            ...
            $(this.elem).progressbar('destroy').empty();
            if ('*' === this.url || _.isArray(this.url)) { return; }
            if (this.template) {
              var template = $('<div>' +
                bt.template(this.template, { settings : this }, this.package) +
                '</div>');
              ...
              if (0 < playTemplate.length) {
                ...
                clickable.click(function() {
                  $(that.elem).html(template.find('.replay'))
                    .click(function(){playClick.call(that);});
                  playClick.call(that);
                });
              }
            }
  ...
  var init = function(settings) {
    ...
    $(this.settings.elem)
      .addClass('bt-widget bt-download-widget')
      .attr('rel', this.settings.url);
  ...
  var Result = (function() {
    var triggerClick = function(widget) {
      var elem = $(widget).find('.clickable');
      if (0 === elem.length) {
        elem = $(widget).find('button,a').first();
      }
      elem.trigger('click');
    };
    ...
    return function(settings) {
      ...
      var that = this, tor;
      try { tor = bt.torrent.get(settings.url); } catch(e) {}
      ...
      if ('undefined' === typeof tor) {
        ...
        if (settings.template) {
          var template = $('<div>' +
            bt.template(settings.template, this, settings.package) + '</div>');
        ...
        if (! templateProvided) {
          var button = $('<button/>').appendTo(this.settings.elem)
            .text(sprintf(this.settings.buttons.download[0],
              this.settings.name))
            .click(function() {
              var settings = that.settings;
              button
              ...
              if ('*' === settings.url) { // Download all
                $('.bt-download-widget').each(function() {
                  if ('*' !== $(this).attr('rel')) { triggerClick(this); }
                });
                that.settings.callbacks.dispatch('addTorrent');
              }
              else if (_.isArray(settings.url)) { // Download Multiple
                $('.bt-download-widget').each(function() {
                  if (-1 < _.indexOf(settings.url, $(this).attr('rel'))) {
                    triggerClick(this);
                  }
                });
              }
        ...
        if (1000 > tor.properties.get('progress') ) {
          ...
          // Download complete. No need for a progress-bar, so go to its
          // callback.
